...
public class FilterForCustomerCreateAccount implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(cname.length()!=0||cun.length()!=0||cemail.length()!=0||cpwd.length()!=0||ccpwd.length()!=0)
		{
			...
			{
				((HttpServletResponse) response).sendRedirect("/CarserviceProject/mismatchCustomerCreateAccount.html");
			}
		}
		else
		{
			((HttpServletResponse) response).sendRedirect("/CarserviceProject/emptyCustomerCreateAccount.html");
